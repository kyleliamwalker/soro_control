...
class HighLevelControl( Node ):
    ...
    def __init__(self):
        ...
        self.seg1_angle_subscriber = self.create_subscription(ArcParam, '/set_arc_parameters_1', self.seg1_set_ten_len_callback, 10)
        self.seg2_angle_subscriber = self.create_subscription(ArcParam, '/set_arc_parameters_2', self.seg2_set_ten_len_callback, 10)
        # assign encoder position
        self.encoder_subscriber = self.create_subscription(Encoder, '/encoder_pos', self.encoder_callback, 10)
        # publish control message
        self.seg1_control_publisher = self.create_publisher(Control, '/control_input_1', 10)
        self.seg2_control_publisher = self.create_publisher(Control, '/control_input_2', 10)
    ...
    def seg1_set_ten_len_callback(self, data):
    ...
    def seg2_set_ten_len_callback(self, data):
        ...
        for i in range(len(L_goal)):
            self.L_change[i+4] = L_goal[i] - self.L[i+4]
            if np.all(self.L_change[i+4] == 0):
                self.steps_to_goal[i+4] = 0
            else:
                self.steps_to_goal[i+4] = self.L_change[i+4]/self.metres_per_step
        ...
        # need to create message here for length change or some other control input
        control_msg = Control()
        control_msg.l1 = self.pos[4] + round(self.steps_to_goal[4])
        control_msg.l2 = self.pos[5] + round(self.steps_to_goal[5])
        control_msg.l3 = self.pos[6] + round(self.steps_to_goal[6])
        control_msg.l4 = self.pos[7] + round(self.steps_to_goal[7])
        #control_msg.l4 = round(self.step_change[3])
        self.seg2_control_publisher.publish(control_msg)
